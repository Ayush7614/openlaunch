...
export async function applySignedEdit(r: EditRequest): Promise<{ ok: true } | { ok: false; error: string; status: number }> {
  ...
  // nonce: exists, matches, unexpired, unused — consumed atomically
  const consumed = await db<{ nonce: string }[]>`
    UPDATE bb_edit_nonces SET used_at = now()
     WHERE nonce = ${r.nonce} AND chain_id = ${cid} AND token = ${token} AND wallet = ${wallet} AND used_at IS NULL AND expires_at > now()
       AND expires_at = ${new Date(expiresAt).toISOString()}
     RETURNING nonce`;
  if (consumed.length === 0) return { ok: false, error: "nonce invalid or already used", status: 401 };

  // signer must be the on-chain launcher
